...
function Benefits(props: Props) {
  ...
  return (
    <div class="relative w-full h-[100vh] z-10 bg-[#240F51]">
      ...
      <div class={`w-full xl:container px-4 md:py-8 flex gap-8 lg:gap-12 lg:py-10 h-full lg:px-0 items-start lg:mx-auto
      `}>
        <div className={`flex ${layout.alignment === "TEXT | IMAGE" ? "flex-col md:flex-row" : "flex-col-reverse md:flex-row-reverse"}
          justify-start md:items-end
        `}>
          <div class={`flex md:w-[70%] ${layout.alignment === "TEXT | IMAGE" ? "mb-7" : "mt-7" } md:mt-0 md:pb-3 md:ml-2`}>
            <Header
              title={title}
          ...
          <div class={`grid md:w-[80%] grid-rows-[1fr_1fr] gap-3 md:gap-7 mx-auto md:mx-0
          ${COLUMNS_GRID_MOBILE[2]} ${COLUMNS_GRID_DESKTOP[layout.columnImages]} `}>
            {list.map((element) => (
