...
export default function TextAboutUs({ text }: Props) {
  ...
  return (
    <div class="flex flex-col gap-4 pb-10 pt-4">
      <div
        class={`transition-[max-height] duration-1000 ${
          isShow.value ? "max-h-[3800px]" : "relative hiddenMiddleTexto"
        } px-5 my-7`}
      >
        <div dangerouslySetInnerHTML={{ __html: text ?? "" }} />
        <div
          class="absolute bottom-0 h-[52px] inline-block w-full left-0"
          style={{
